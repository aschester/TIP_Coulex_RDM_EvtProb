...
CsI_detector::CsI_detector(G4LogicalVolume* experimentalHall_log,Materials* mat)
{
  ...
  CsI_absorber_thickness=2.0*um; // 0.276 mg/cm^2
...
G4VPhysicalVolume* CsI_detector::Construct()
{
  ...
  G4Colour csi_yellow (0.639,1.0,0.0);
  G4VisAttributes* Vis_6 = new G4VisAttributes(csi_yellow);
  Vis_6->SetVisibility(true);
  Vis_6->SetForceSolid(true);

  CsI_detector_log->SetVisAttributes(Vis_6);

  G4Colour al_blue (0.678,0.698,0.741); 
  G4VisAttributes* Vis_7 = new G4VisAttributes(al_blue);
  Vis_7->SetVisibility(false);
