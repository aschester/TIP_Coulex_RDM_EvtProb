...
void DetectionSystemGriffin::ConstructNewHeavyMet()
{
  ...
  if(elW == NULL)
    elW = new G4Element("Tungsten","W",  74., 183.84*g/mole);
  if(elNi == NULL)
    elNi = new G4Element("Nickel" ,"Ni", 28., 58.69*g/mole);
  if(elCu == NULL)
    elCu = new G4Element("Copper" ,"Cu", 29., 63.54*g/mole);
  if(materialHevimetal == NULL) {
    materialHevimetal = new G4Material("Heavimetal",17.0*g/cm3,3);
  ...
  if( !materialHevimetal ) {
    G4cout << " ----> Material " << this->crystal_material << " not found, cannot build the heavimetal collimator! " << G4endl;
    exit(1);
  }
